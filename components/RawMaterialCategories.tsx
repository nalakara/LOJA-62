...
const RawMaterialCategories: React.FC<RawMaterialCategoriesProps> = ({ categories, onEdit, onDelete }) => {
  ...
  return (
    <div className="bg-bone-light border border-mineral rounded-2xl shadow-sm p-6">
      <h2 className="text-2xl font-bold text-ink mb-6">{t('rawMaterialCategoryManagementTitle')}</h2>
      <div className="overflow-x-auto">
       ...
       {categories.length === 0 && (
          <div className="text-center py-10 text-ink-muted">
              <p className="font-semibold">{t('noMaterialCategoriesFound')}</p>
              <p className="text-sm text-ink-faint">{t('pleaseAddMaterialCategory')}</p>
          </div>
      )}
    </div>
  );
